...
app.post('/api/chat/send', validateOrigin, async (req, res) => {
  try {
    ...
    // Respuesta simple de fallback
    const response = 'Gracias por tu mensaje. Un asesor te contactará pronto.';

    // Simular tiempo de procesamiento
    await new Promise(resolve => setTimeout(resolve, 1000));

    console.log('[UCV Chat Fallback] Sending response:', response);
...
// Iniciar servidor
app.listen(PORT, () => {
  console.log(`[UCV] Voice Widget API Server running on port ${PORT}`);
  console.log(`[UCV] ElevenLabs configured: ${!!(ELEVENLABS_API_KEY && ELEVENLABS_AGENT_ID)}`);
  console.log(`[UCV] Allowed domains: ${ALLOWED_DOMAINS.join(', ')}`);
});
